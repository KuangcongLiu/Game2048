...
public class Board
{
  ...
  private void moveRight()
  {
    ...
    for(int i=GRID_SIZE-1;i>=0;i--){
      ...
      for(int m=0;m<GRID_SIZE-1;m++){
        ...
        for(int j=GRID_SIZE-2;j>=0;j--){
          ...
          if(grid[i][j+1]==0){
            ...
            //and assign the value of the original tile to be 0
            grid[i][j]=0;
          }
        }
      }
      ...
      for(int j=GRID_SIZE-2;j>=0;j--){
        //if there is two tiles with the same value,
        if(grid[i][j]==grid[i][j+1]){
          //we should assign the rightside tile to be the sum
      ...
      //as we added some tiles together, we now have some new empty
      //tiles between, so we should move them to the right again.
      //But this time, there should only be one empty tile between
      //every two non-empty tiles, so moving once is enough.
      for(int j=GRID_SIZE-2;j>=0;j--){
